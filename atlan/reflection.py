...
# 8. SelfAnchor (Uses Dreamspace feedback for Beliefs)
class SelfAnchor:
    ...
    def get_state(self) -> Dict[str, Any]:
        """Returns anchor state."""
        return {
            "reflection_log": self.reflection_log,
            "action_feedback_log": self.action_feedback_log
        }

    def load_state(self, state: Dict[str, Any]):
        """Restores anchor state."""
        self.reflection_log = state.get("reflection_log", [])
        self.action_feedback_log = [tuple(x) for x in state.get("action_feedback_log", [])]
