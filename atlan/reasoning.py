...
# 5. BeliefSystem (Adapted from belief.py)
class BeliefSystem:
    ...
    def revise_beliefs(self) -> List[Tuple[str, str]]:
        ...
        for stmt in list(self.beliefs.keys()):
            if stmt in self.beliefs:
                # Check if not already revised in this pass
...
class DreamspaceSimulator:
    ...
    def simulate(self, memory_index: Any, start_node_indices: List[int], steps: int = 5) -> List[str]:
        """
        Runs a 'Mental Movie' (Causal Simulation) starting from the given nodes.
        Propagates activation forward along CAUSAL edges (A -> B).
        """
        timeline = []
        
        # Initial State
        current_indices = start_node_indices
        
        # Record start
        start_phrases = [memory_index.memory[i].phrase for i in current_indices if i < len(memory_index.memory)]
        timeline.append(f"Start: {', '.join(start_phrases)}")
        ...
        for step in range(steps):
            ...
            for idx in current_indices:
                if idx >= len(memory_index.memory): continue
                
                node = memory_index.memory[idx]
                
                # Find strongest causal successors
                # causal_edges is Dict[target_uuid, weight]
                # We need to map UUID back to index. 
                # Optimization: This is slow without a UUID map.
                # But for simulation (small scale), it's acceptable.
                
                successors = []
                for target_id, weight in node.causal_edges.items():
                    # Find target index
                    target_idx = -1
                    for i, n in enumerate(memory_index.memory):
                        if n.node_id == target_id:
                            target_idx = i
                            break
                    
                    if target_idx != -1:
                        successors.append((target_idx, weight))
                
                # Sort by weight
                successors.sort(key=lambda x: x[1], reverse=True)
                
                # Pick top 1-2 successors
                for next_idx, w in successors[:3]:
                    if w > 0.15: # Threshold
                        phrase = memory_index.memory[next_idx].phrase
                        # Ignore stopwords (hardcoded for now, or pass stop_words set)
                        if phrase in {"the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "of", "is", "gets", "come", "fly"}:
                            continue
                            
                        next_indices.append(next_idx)
                        step_phrases.append(phrase)
            
            if not next_indices:
                break
                
            # Deduplicate
            next_indices = list(set(next_indices))
            step_phrases = list(set(step_phrases))
            
            timeline.append(f"Step {step+1}: {', '.join(step_phrases)}")
